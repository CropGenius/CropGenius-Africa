...
export class CropIntelligenceEngine {
  ...
  async analyzeCropImage(
    ...
    try {
      ...
      const proxyResponse = await fetch(`${SUPABASE_URL}/functions/v1/crop-disease-proxy`, {
        method: 'POST',
        headers: {
      ...
      if (!proxyResponse.ok) {
        ...
        throw new Error(`Crop disease proxy error ${proxyResponse.status}: ${errorText}`);
      }

      const plantData = await proxyResponse.json();
