...
import { MapPin, Target, ArrowLeft } from 'lucide-react';
...
interface StepTwoProps {
  ...
  onLocationChange: (location: Coordinates, description?: string) => void;
...
export default function StepTwo({
  ...
  const getCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        // Get location name first, then call onLocationChange with description
        fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${newLocation.lng},${newLocation.lat}.json?access_token=${import.meta.env.VITE_MAPBOX_ACCESS_TOKEN}`)
          ...
          .then(data => {
            if (data.features && data.features.length > 0) {
              const description = data.features[0].place_name;
              setSearchedLocation(description);
              onLocationChange(newLocation, description);
            } else {
  ...
  const handleUseRecentLocation = (recent: { name: string, coordinates: Coordinates }) => {
    onLocationChange(recent.coordinates, recent.name);
