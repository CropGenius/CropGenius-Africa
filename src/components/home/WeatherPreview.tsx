...
export default function WeatherPreview() {
  ...
  const fetchWeatherData = async (lat: number, lon: number) => {
    try {
      ...
      // Fetch real weather data from Supabase
      const { data: weatherData, error: weatherError } = await supabase
        .from('weather_cache')
        .select('*')
        .eq('lat', Math.round(lat * 100) / 100)
        .eq('lon', Math.round(lon * 100) / 100)
      ...
      // Transform database data to WeatherPreview format
      const currentWeather = weatherData || {
        current: {
          ...
          farm_action: "Apply foliar fertilizer",
          urgency: "normal"
        }
      };

      const realWeather: WeatherPreview = {
        temp: Math.round(currentWeather.current?.temp || 25),
        condition: currentWeather.current?.condition || "Partly Cloudy",
        icon: getWeatherIconFromCondition(currentWeather.current?.condition || "Partly Cloudy"),
        rainChance: currentWeather.current?.rain_chance || 20,
        humidity: currentWeather.current?.humidity || 65,
        windSpeed: currentWeather.current?.wind_speed || 12,
        recommendation: currentWeather.current?.recommendation || "Good conditions for field work",
        farmAction: currentWeather.current?.farm_action || "Apply foliar fertilizer",
        urgency: (currentWeather.current?.urgency as "normal" | "warning" | "critical") || "normal",
        forecast: (forecastData || []).map((item, index) => ({
          day: getDayName(index),
          temp: Math.round(item.forecast?.temp || 25 + index),
          icon: getWeatherIconFromCondition(item.forecast?.condition || "Partly Cloudy"),
          rainChance: item.forecast?.rain_chance || 20
        }))
      };
      ...
      if (realWeather.urgency === "critical") {
        setTimeout(() => {
          toast.warning("Weather Alert Detected", {
            description: realWeather.farmAction,
            action: {
              label: "View Details",
              onClick: () => {}
            }
      ...
      setWeather({
        ...
        windSpeed: 12,
        recommendation: "Good conditions for field work",
        farmAction: "Apply foliar fertilizer",
        urgency: "normal",
        forecast: [
          { day: "Today", temp: 25, icon: "cloud", rainChance: 20 },
          { day: "Tomorrow", temp: 26, icon: "sun", rainChance: 10 },
          { day: getDayName(2), temp: 24, icon: "cloud", rainChance: 30 },
          { day: getDayName(3), temp: 27, icon: "sun", rainChance: 15 },
          { day: getDayName(4), temp: 28, icon: "sun", rainChance: 5 }
        ]
      });
      setLoading(false);
    }
  ...
  const getDayName = (offset: number) => {
    const days = ["Today", "Tomorrow", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
    return days[offset] || "Day" + offset;
  };

  const getWeatherIconFromCondition = (condition: string): "sun" | "cloud" | "rain" | "storm" => {
