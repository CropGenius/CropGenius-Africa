...
export default function MarketPreview() {
  ...
  const fetchMarketData = async () => {
    try {
      ...
      // Transform database data to CropPricing format
      const realMarketData: CropPricing[] = (marketData || []).slice(0, 3).map(item => ({
        name: item.commodity,
        currentPrice: item.price,
        change: item.price_change_percent || 0,
        trend: item.price_change_percent > 0 ? "up" : item.price_change_percent < 0 ? "down" : "stable",
        aiRecommendation: item.ai_recommendation || `Current market conditions for ${item.commodity} are ${item.demand_level} demand level.`,
        action: item.ai_recommendation?.toLowerCase().includes("sell") ? "sell" : 
               item.ai_recommendation?.toLowerCase().includes("buy") ? "buy" : "hold",
        urgency: item.demand_level === "high" && item.price_change_percent > 5 ? "high" : 
                item.demand_level === "low" && item.price_change_percent < -5 ? "medium" : "low",
        prediction: item.ai_forecast ? {
          days: 7,
          price: item.price + (item.price * (item.price_change_percent || 0) / 100),
          reason: item.ai_forecast
        } : undefined
      }));
      ...
      const urgentCrops = realMarketData.filter(c => c.urgency === 'high');
      ...
      setMarketData([
        {
          ...
          change: 3.8,
          trend: "up",
          aiRecommendation: "Loading market data...",
          action: "hold",
          urgency: "medium"
        }
      ]);
