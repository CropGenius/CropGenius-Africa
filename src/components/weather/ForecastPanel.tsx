  ...
  useEffect(() => {
    if (apiForecast && apiForecast.length > 0) {
      ...
      const processedForecast = apiForecast.map(day => ({
        ...
        icon: getConditionIcon(day.condition, day.icon),
        windSpeed: Math.round(day.windSpeed),
        humidity: day.humidity,
        hourly: generateHourlyForecast(day),
        farmAction: getFarmAction(day.condition, day.rainProb)
      }));
          ...
          <div className="lg:col-span-2">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-8 gap-2">
              {selectedDay.hourly.map((hour: any, index: number) => (
                <div key={`${hour.hour}-${index}`} className="bg-slate-50 dark:bg-slate-900 rounded-lg p-2 text-center">
