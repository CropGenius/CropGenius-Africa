...
serve(async (req) => {
  ...
  try {
    let phone: string, message: string, userId: string, insightType: "weather" | "market" | "pest" | "fertilizer";
    ...
    if (req.method === 'POST') {
      ...
      // GET request - parse from URL parameters for testing
      const url = new URL(req.url);
      phone = url.searchParams.get('phone') || '+1234567890';
      message = url.searchParams.get('message') || 'Test message from CropGenius';
      userId = url.searchParams.get('userId') || 'test-user';
      insightType = (url.searchParams.get('insightType') || 'weather') as "weather" | "market" | "pest" | "fertilizer";
    }
    ...
    if (!userMemory.whatsappOptIn) {
      return new Response(
        JSON.stringify({ success: false, message: "User has not opted in to WhatsApp notifications" }),
